...
namespace SalesFlow.Business.Validations.CustomerValidators
{
    public class CreateCustomerValidator : AbstractValidator<CreateCustomerDto>
    {
        public CreateCustomerValidator()
        {
            ...
            RuleFor(x => x.ContactFirstName).Cascade(CascadeMode.Stop).NotEmpty().WithMessage("First name is required.").MaximumLength(50);

            RuleFor(x => x.ContactLastName).Cascade(CascadeMode.Stop).NotEmpty().WithMessage("Last name is required.").MaximumLength(50);
            ...
            {
                RuleFor(x => x.Website).MaximumLength(200).Must(url => Uri.TryCreate(url, UriKind.Absolute, out _)).WithMessage("Website must be a valid URL.");
            });
